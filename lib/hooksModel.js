...
module.exports = (Sequelize, patches) => {
	...
	function* beforeCreate(item, options) {
		...
		const parent = yield findOne(
			model,
			addOptions({where: {[primaryKey]: parentId}, attributes: [levelFieldName], useMaster: true}, options)
		);
		...
		if (parentId === null) {
			...
			if (!parent) {
				parent = yield findOne(
					model,
					addOptions({
						where: {[primaryKey]: parentId}, attributes: [levelFieldName, foreignKey], useMaster: true
					}, options)
				);
				if (!parent) throw new HierarchyError('Parent does not exist');
			}
